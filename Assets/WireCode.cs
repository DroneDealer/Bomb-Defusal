...
public class WireCode : MonoBehaviour
{
    ...
    void NewTargetZone()
    {
        ...
        MoveZoneToRandomXPosition(redZone);
        MoveZoneToRandomXPosition(yellowZone);
        MoveZoneToRandomXPosition(greenZone);
    ...
    void MoveZoneToRandomXPosition(RectTransform zone)
    {
        float minX = wireBounds.rect.xMin + (zone.rect.width / 2f);
        float maxX = wireBounds.rect.xMax - (zone.rect.width / 2f);
        float newx = Random.Range(minX, maxX);
        Vector2 newPosition = zone.anchoredPosition;
        newPosition.x = newx;
        zone.anchoredPosition = newPosition;
    }
